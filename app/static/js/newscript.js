...
function toggleDeleteMethod() {
    ...
    if (methodSelect.value === 'visual') {
        ...
        // If file is already selected, automatically load previews
        if (fileInput.files[0]) {
            loadDeletePagePreviews(fileInput.files[0]);
        }
    } else {
    ...
    if (fileInput.files[0]) {
        ...
        fileReader.onload = function () {
            ...
            pdfjsLib.getDocument({ data: arrayBuffer }).promise.then(function (pdf) {
                ...
                // Auto-load previews if visual method is selected
                const methodSelect = document.getElementById('deletePages-method');
                if (methodSelect && methodSelect.value === 'visual') {
                    loadDeletePagePreviews(file);
                }
            });
        };
...
// Helper functions for page selection - NO CHANGES NEEDED
...
// Main processing function for selected pages - NO CHANGES NEEDED
...
//     const form = document.getElementById('deletePagesForm');
//     const fileInput = document.getElementById('deletePages-file');
//     const deleteType = document.getElementById('deletePages-type');
//     const specificInput = document.getElementById('deletePages-specific');
//     const rangeInput = document.getElementById('deletePages-range');
...
//     try {
//         // First, load and display page previews
//         await loadDeletePagePreviews(file);

//         // Show the page selection interface
//         showPageSelectionInterface();

//     } catch (error) {
//         console.error('Page preview loading failed:', error);
//         // Fall back to original text-based method
//         proceedWithOriginalDeletionMethod(file, deleteType, specificInput, rangeInput, resultDiv, progressDiv, progressText, submitButton);
//     }
// }

// // Function to load page previews for deletion selection
// async function loadDeletePagePreviews(file) {
//     const previewContainer = document.getElementById('delete-pages-preview-container');
...
//         newContainer.id = 'delete-pages-preview-container';
//         newContainer.className = 'mt-6 border-t pt-6';

//         const form = document.getElementById('deletePagesForm');
//         const originalFields = form.querySelector('.original-fields');
//         if (originalFields) {
//             form.insertBefore(newContainer, originalFields.nextSibling);
//         } else {
//             form.appendChild(newContainer);
//         }
//     }

//     const container = document.getElementById('delete-pages-preview-container');
//     container.innerHTML = `
//         <div class="mb-4">
//             <h3 class="text-lg font-semibold text-gray-800 mb-3">Select Pages to Delete</h3>
//             <p class="text-sm text-gray-600 mb-4">Click on pages to select/deselect them for deletion. Selected pages will be highlighted in red.</p>
...
//                 <button type="button" id="use-text-method" class="text-blue-600 hover:text-blue-800 text-sm underline">
//                     Or use text-based page selection instead
//                 </button>
//             </div>
//         </div>
//     `;
...
//             const page = await pdf.getPage(i);
//             const viewport = page.getViewport({ scale: 0.2 }); // Small scale for grid
...
//             processSelectedPagesDeletion(file)
//         );
//         document.getElementById('use-text-method').addEventListener('click', () => 
//             showTextBasedMethod()
//         );
...
// async function proceedWithOriginalDeletionMethod(file, deleteType, specificInput, rangeInput, resultDiv, progressDiv, progressText, submitButton) {
//     console.log('Using text-based page deletion method...');

//     let pagesToDelete = [];

//     // Get pages to delete based on type
//     if (deleteType.value === 'specific') {
//         if (!specificInput.value) {
//             alert('Please enter pages to delete (e.g., 1,3,5)');
//             return;
//         }
//         pagesToDelete = specificInput.value.split(',').map(p => parseInt(p.trim()) - 1); // Convert to 0-based
//     } else {
//         if (!rangeInput.value) {
//             alert('Please enter page range (e.g., 2-5)');
//             return;
//         }
//         const rangeMatch = rangeInput.value.match(/^(\d+)-(\d+)$/);
//         if (!rangeMatch) {
//             alert('Invalid range format. Use format like 2-5');
//             return;
//         }
...
//         // Validate page numbers
//         if (pagesToDelete.some(page => page < 0 || page >= totalPages)) {
//             throw new Error(`Invalid page numbers. PDF has ${totalPages} pages.`);
//         }

//         console.log(`Deleting pages: ${pagesToDelete.map(p => p + 1).join(', ')}`);
...
//         resultDiv.innerHTML = `
//             <div class="text-green-600">
//                 ✅ <strong>Pages Deleted Successfully!</strong><br>
//                 📁 File: ${filename}<br>
//                 🗑️ Pages deleted: ${pagesToDelete.length}<br>
//                 📄 Remaining pages: ${pageIndices.length}<br>
//             </div>
//         `;
...
//         resultDiv.innerHTML = `
//             <div class="text-red-600">
//                 ❌ Page deletion failed: ${error.message}<br>
//                 <small>Falling back to server processing...</small>
//             </div>
//         `;
...
// // UI Helper functions
// function showPageSelectionInterface() {
//     // Hide the original input fields
//     const originalFields = document.querySelectorAll('#deletePagesForm .original-fields');
//     originalFields.forEach(field => {
//         field.style.display = 'none';
//     });

//     // Hide progress during selection
//     const progressDiv = document.getElementById('progress-deletePagesForm');
//     if (progressDiv) {
//         progressDiv.style.display = 'none';
//     }
...
// .page-preview-item.border-red-500 {
...
// #page-previews-grid::-webkit-scrollbar {
//     width: 8px;
// }

// #page-previews-grid::-webkit-scrollbar-track {
//     background: #f7fafc;
//     border-radius: 4px;
// }

// #page-previews-grid::-webkit-scrollbar-thumb {
//     background: #cbd5e0;
//     border-radius: 4px;
// }

// #page-previews-grid::-webkit-scrollbar-thumb:hover {
//     background: #a0aec0;
// }
...
// // Inject styles
// if (!document.getElementById('delete-pages-styles')) {
//     const styleSheet = document.createElement('style');
//     styleSheet.id = 'delete-pages-styles';
//     styleSheet.textContent = deletePagesStyles;
...
// KEEP THIS ONE (remove any others):
document.addEventListener('DOMContentLoaded', function () {
    // Set the dropdown to text method by default
    const methodSelect = document.getElementById('deletePages-method');
    if (methodSelect) {
