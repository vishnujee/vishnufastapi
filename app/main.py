...
ALLOWED_FILE_TYPES = {
    'pdf': {
        'mime': ['application/pdf', 'application/octet-stream'],  # octet-stream for browser-generated PDFs
        'extensions': ['.pdf'],
        'max_size': 50 * 1024 * 1024,  # 50MB
        'magic': [b'%PDF']
    },
    'image': {
        'mime': ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/jpg', 'image/x-ms-bmp'],
...
async def check_ws_rate_limit(client_ip: str, max_connections: int = 5, window_seconds: int = 60) -> bool:
    now = datetime.now()
    ws_connections[client_ip] = [ts for ts in ws_connections[client_ip] 
...
async def security_health_check(request: Request):
    ...
    status = {
        ...
        "file_validation": {
            ...
            "max_image_size_mb": ALLOWED_FILE_TYPES['image']['max_size'] // (1024*1024),
            "max_video_size_mb": ALLOWED_FILE_TYPES['video']['max_size'] // (1024*1024),
        },
...
def init_security():
    ...
    init_clamav_scanner()
    logger.info(f"✅ Security initialized - ClamAV: {'Available' if CLAMAV_AVAILABLE else 'Not Available'}")
...
def is_raw_table_text(text):
    ...
    return (number_count >= 3 and has_tabular and not has_header_keywords) or (has_tabular and not has_header_keywords)

def is_table_title(line, title_keywords=TITLE_KEYWORDS):
    if not line.strip() or len(line) > 100: return False
    if re.search(r'^\d+\s', line) or re.search(r'\s\d+\.\d+\s', line): return False
...
def is_header_row(row_str, header_keywords=HEADER_KEYWORDS, min_matches=1):
    if not row_str.strip(): return False
    lower_row = row_str.lower()
    matches = sum(1 for kw in header_keywords if kw.lower() in lower_row)
    if "project name" in lower_row or "duration" in lower_row or "company" in lower_row: return True
...
def is_substring_match(line, table_content_set):
    ...
    for table_content in table_content_set:
        if norm_line == table_content: return True
        if len(norm_line) > 15 and len(table_content) > 15:
            if norm_line.lower() in table_content.lower(): return True
    return False
...
def post_process_retrieved_docs(docs, query):
    ...
    for doc in docs:
        ...
        is_work_table = ("3.pdf" in source and content.count("|") > 3 and any(header in content for header in table_headers))
        if is_work_table:
...
def ensure_tabular_inclusion(docs, query, min_tabular=2):
    ...
    is_work_query = any(keyword in query_lower for keyword in ['company', 'work', 'experience', 'job', 'project', 'started working', 'career', 'professional', 'employment', 'when did', 'start date', 'kei', 'larsen', 'toubro', 'vindhya', 'punj', 'gng', 'l&t','vtl','l&t'])
    if is_work_query:
        ...
        website_docs = [d for d in docs if any(keyword in d.page_content.lower() for keyword in ['recallmind', 'parcelfile', 'vishnuji.com', 'website', 'file transfer'])]
...
class ChromaDBRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query: str, *, run_manager = None) -> List[Document]:
        try:
            ...
            docs_with_scores = self.vectorstore.similarity_search_with_score(query, k=self.search_kwargs["k"])
            docs = []
        ...
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            ...
            for page_num in range(1, total_pages + 1):
                ...
                if not original_text.strip():
                    all_page_chunks.append({'content': f"Page {page_num} - No extractable content", 'page_num': page_num, 'chunk_num': 1, 'total_chunks_page': 1, 'content_hash': hashlib.md5("empty".encode()).hexdigest()})
                    continue
                logger.info(f"   📝 Page {page_num}: {len(original_text)} characters")
                page_chunks = process_single_page_complete(original_text, page_num, pdf_url, max_chunks_per_page, target_chunk_size)
...
def process_single_page_complete(original_text, page_num, pdf_url, max_chunks, target_size):
    ...
    chunks = create_content_preserving_chunks(sections, max_chunks, target_size, page_num)
    page_chunks = []
    for chunk_num, chunk_content in enumerate(chunks, 1):
        page_chunks.append({'content': chunk_content, 'page_num': page_num, 'chunk_num': chunk_num, 'total_chunks_page': len(chunks), 'pdf_source': pdf_url.split('/')[-1], 'content_hash': hashlib.md5(chunk_content.encode()).hexdigest()})
    return page_chunks
...
def should_preserve_section(lines):
    ...
    first_line = lines[0]
    is_heading_like = (first_line.endswith(':') or (len(first_line) < 100 and first_line and first_line[0].isalnum()))
    if is_heading_like:
...
def create_content_preserving_chunks(sections, max_chunks, target_size, page_num):
    ...
    for section in sections:
        ...
        if section_size > ideal_chunk_size:
            ...
            continue
        if (current_size + section_size > ideal_chunk_size and current_chunk and len(chunks) < max_chunks - 1):
            chunks.append(' '.join(current_chunk))
...
def split_large_section(section, max_size):
    if len(section) <= max_size: return [section]
    chunks = []
    words = section.split()
    current_chunk = []
    current_size = 0
    ...
    if original_normalized != all_chunks_normalized:
        ...
        if missing_words:
            ...
            for sentence in original_sentences:
                ...
                if (sentence and len(sentence) > 20 and sentence not in all_chunks_normalized):
            ...
            with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
                ...
                for page_num in range(1, total_pages + 1):
                    ...
                    if not original_text.strip():
                        ...
                        continue
                    page_chunks = process_single_page_complete(original_text, page_num, pdf_url, 3, 2000)
...
#         existing_vectorstore = Chroma(collection_name="vishnu_ai_docs", embedding_function=embeddings, persist_directory=persist_dir)
...
#         vectorstore = Chroma(collection_name="vishnu_ai_docs", embedding_function=embeddings, persist_directory=persist_dir)
#         all_documents = []
...
#                     tabular_doc = Document(page_content=tabular_text, metadata={"source": pdf_url, "content_type": "mixed_text_and_tables", "document_type": "tabular", "section": "work_experience", "page_num": 1, "chunk_num": 1, "total_chunks_page": 1})
#                     all_documents.append(tabular_doc)
#                     logger.info(f"✅ Tabular PDF processed: {len(tabular_text)} characters")
#                 else:
#                     logger.info(f"📝 Processing: {pdf_name}")
#                     page_chunks = process_non_tabular_pdf_complete(pdf_bytes, pdf_url, max_chunks_per_page=3, target_chunk_size=2500)
#                     logger.info(f"📑 Created {len(page_chunks)} chunks from {pdf_name}")
#                     for chunk_info in page_chunks:
#                         chunk_doc = Document(page_content=chunk_info['content'], metadata={"source": pdf_url, "content_type": "text_heavy", "document_type": "nontabular", "page_num": chunk_info['page_num'], "chunk_num": chunk_info['chunk_num'], "total_chunks_page": chunk_info['total_chunks_page'], "pdf_source": chunk_info['pdf_source'], "content_hash": chunk_info['content_hash']})
...
#             fallback_doc = Document(page_content="Vishnu Kumar - Electrical Engineer with 12 years experience. Worked at L&T, KEI Industries, Punj Lloyd. Skills: substation execution, project management, quality assurance.", metadata={"source": "fallback", "content_type": "text", "document_type": "fallback", "page_num": 1, "chunk_num": 1})
...
#         try:
#             embeddings = HFEmbeddings()
#             vectorstore = Chroma(collection_name="vishnu_ai_docs_fallback", embedding_function=embeddings, persist_directory="./chroma_db_fallback")
#             fallback_doc = Document(page_content="System is initializing. Please try chat functionality.", metadata={"source": "fallback", "error": True})
...
def initialize_vectorstore():
    try:
        ...
        # FIX: Check if database is corrupted and recreate if needed
        recreate_db = False
        try:
            # Test if existing DB works
            test_vectorstore = Chroma(
                collection_name="vishnu_ai_docs", 
                embedding_function=embeddings, 
                persist_directory=persist_dir
            )
            # Try to access collection - this will fail if schema is wrong
            test_vectorstore._collection.count()
            logger.info(f"✅ Existing ChromaDB is healthy")
        except Exception as db_error:
            if "no such column" in str(db_error) or "OperationalError" in str(db_error):
                logger.warning(f"⚠️ ChromaDB schema issue detected: {db_error}")
                logger.warning("🔄 Will recreate database from scratch")
                recreate_db = True
            else:
                logger.error(f"❌ Unknown ChromaDB error: {db_error}")
                recreate_db = True
        ...
        if recreate_db:
            ...
            if os.path.exists(persist_dir):
                ...
                logger.info(f"📦 Backing up corrupted DB to {backup_dir}")
                shutil.move(persist_dir, backup_dir)
            os.makedirs(persist_dir, exist_ok=True)
            logger.info("✅ Created fresh ChromaDB directory")
        
        # Now try to load or create fresh
        try:
            ...
            doc_count = existing_vectorstore._collection.count()
            if doc_count > 0:
                logger.info(f"✅ Using existing ChromaDB with {doc_count} documents")
                return existing_vectorstore, embeddings
        except Exception as e:
            logger.warning(f"Could not load existing DB, creating new one: {e}")
        ...
        try:
            # Clean up any corrupted fallback DB
            fallback_dir = "./chroma_db_fallback"
            ...
            logger.info("✅ Fallback ChromaDB created successfully")
            return vectorstore, embeddings
        ...
        while True:
            ...
            if message_data.get("type") == "heartbeat":
                ...
                if target in p2p_manager.active_connections:
                    ...
                    await websocket.send_json({"type": "error", "message": f"Peer {target} not connected"})
...
async def test_websocket():
    return {"active_connections": list(p2p_manager.active_connections.keys()), "total_connections": len(p2p_manager.active_connections), "status": "running"}
...
async def startup_event():
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            retriever = ChromaDBRetriever(vectorstore=vectorstore, search_kwargs={"k": 10})
            ...
            if attempt < max_retries:
                ...
                class FallbackRetriever(BaseRetriever):
                    def _get_relevant_documents(self, query: str, *, run_manager = None) -> List[Document]:
                        return [Document(page_content="System is experiencing technical difficulties. Please try again later.", metadata={"source": "fallback", "error": True})]
...
async def api_login(request: Request, credentials: dict, response: Response, background_tasks: BackgroundTasks):
    username = credentials.get("username", "").strip()
    password = credentials.get("password", "").strip()
    if username == "admin" and password == os.getenv("CLEANUP_DASHBOARD_PASSWORD"):
        session_token = create_session_token(username)
        active_sessions[session_token] = {"username": username, "login_time": time.time(), "authenticated": True}
        user_agent = request.headers.get("user-agent", "").lower()
        is_mobile = any(term in user_agent for term in ['mobile', 'android', 'iphone', 'ipad'])
        if is_mobile:
            redirect_url = f"/cleanup?token={session_token}&mobile=true&ts={int(time.time())}"
            response = RedirectResponse(url=redirect_url, status_code=303)
            response.set_cookie(key="session_token", value=session_token, max_age=3600, httponly=False, samesite="lax", secure=False)
            return response
        else:
            response_data = {"status": "success", "token": session_token, "message": "Login successful"}
...
async def cookie_test(request: Request, response: Response):
    response = JSONResponse({"message": "Cookie test", "cookies_received": request.cookies, "headers": dict(request.headers)})
    response.set_cookie(key="test_cookie", value="working_" + str(time.time()), max_age=3600, httponly=False, samesite="none", secure=False)
    return response
...
async def get_cleanup_logs(request: Request):
    ...
    try:
        ...
        if not os.path.exists(log_path):
            return {"logs": "No logs found yet. Cronjob may not have run.", "last_updated": None, "file_size": "0 KB", "line_count": 0, "recent_entries": []}
        ...
        return {"logs": "\n".join(lines_reversed), "last_updated": last_updated, "file_size": f"{stat.st_size / 1024:.2f} KB", "line_count": len(lines), "recent_entries": recent_entries, "total_runs": total_runs, "next_run_ist": next_run.strftime("%H:%M:%S"), "current_time_ist": now_ist.strftime("%Y-%m-%d %H:%M:%S")}
    except Exception as e:
...
async def test_cleanup(request: Request):
    ...
    try:
        ...
        return {"message": "Cleanup completed successfully", "timestamp": time.time(), "status": "success"}
    except Exception as e:
...
async def session_debug(request: Request):
    ...
    if cookie:
        try:
            ...
            pass
    return {"cookie_exists": bool(cookie), "cookie_data": cookie_data, "all_cookies": request.cookies, "user_agent": request.headers.get("user-agent"), "is_mobile": "Mobile" in request.headers.get("user-agent", "")}
        ...
        for dir_name, base_dir in directories.items():
            if not base_dir.exists():
                stats["directories"][dir_name] = {
                    "total_files": 0, "old_files": 0, "total_size": 0,
                    "exists": False, "path": str(base_dir), "files": []
                }
                continue
...
async def get_backend_logs(request: Request):
    ...
    try:
        if not BACKEND_LOG_PATH.exists():
            return {"file_size": "0 KB", "last_updated": None, "total_entries": 0, "recent_entries": ["No backend.log file found"], "logs": "No backend.log file found", "all_logs": []}
        ...
        return {"file_size": f"{file_size_kb:.1f} KB", "last_updated": stat.st_mtime, "total_entries": len(all_lines), "recent_entries": recent_lines, "logs": "\n".join(all_logs_reversed), "all_logs": all_logs_reversed}
    except Exception as e:
...
async def async_retrieve_documents(query: str, retriever, max_timeout: float = 6.0):
    try:
        ...
        docs = await asyncio.wait_for(loop.run_in_executor(thread_pool, lambda: retriever.invoke(query) if retriever else []), timeout=max_timeout)
        return docs
...
async def chat(request: Request, query: str = Form(...), mode: str = Form(None), history: str = Form(None)):
            ...
            if mode and mode in CHAT_MODES:
                ...
                for chunk in response:
                    if chunk.text:
                        ...
                        current_time = time.time()
                        if (buffer_size >= MAX_BUFFER_SIZE or (current_time - last_flush_time) >= MAX_BUFFER_TIME):
                            if chunk_buffer:
                ...
                completion_data = json.dumps({'chunk': '', 'done': True, 'full_response': full_response, 'mode': 'direct', 'timings': timings})
                ...
                if not processed_docs:
                    ...
                    try:
                        ...
                        response = GEMINI_MODEL.generate_content(optimized_prompt, stream=True, generation_config=generation_config, request_options={'timeout': 25})
                        connection_time = time.time() - connection_start
                        full_response = ""
                        chunk_count = 0
                        total_chars = 0
                        ...
                        completion_data = json.dumps({'chunk': '', 'done': True, 'full_response': full_response, 'mode': 'rag', 'timings': timings, 'retrieved_docs_count': len(raw_docs), 'processed_docs_count': len(processed_docs)})
                        yield f"data: {completion_data}\n\n"
                    except asyncio.TimeoutError:
...
compression_presets = {"screen": {"dpi": 72, "quality": "screen", "desc": "Low quality, smallest size"}, "ebook": {"dpi": 150, "quality": "ebook", "desc": "Medium quality, good compression"}, "printer": {"dpi": 300, "quality": "printer", "desc": "High quality for printing"}, "prepress": {"dpi": 300, "quality": "prepress", "desc": "Highest quality, minimal compression"}}
...
def compress_pdf_ghostscript_file(input_path: str, output_path: str, compression_level: str = "ebook"):
    compression_settings = {"screen": "/screen", "ebook": "/ebook", "printer": "/printer", "prepress": "/prepress"}
    ...
    if platform.system() == "Windows":
        ...
        creationflags = 0
    gs_command = base_cmd + ["-dNOPAUSE", "-dBATCH", "-dQUIET", "-sDEVICE=pdfwrite", f"-dPDFSETTINGS={compression_settings[compression_level]}", "-dCompatibilityLevel=1.4", "-dDetectDuplicateImages=true", "-dCompressFonts=true", "-dEmbedAllFonts=true", "-dSubsetFonts=true", "-dColorImageDownsampleType=/Average", "-dGrayImageDownsampleType=/Average", "-dMonoImageDownsampleType=/Subsample", "-dColorImageResolution=120", "-dGrayImageResolution=120", "-dMonoImageResolution=200", "-dAutoFilterColorImages=true", "-dAutoFilterGrayImages=true", "-dColorImageFilter=/DCTEncode", "-dGrayImageFilter=/DCTEncode", "-dCompressPages=true", "-dMaxPatternBitmap=5000000", "-dBufferSpace=80000000", "-dMaxBitmap=50000000", "-dNumRenderingThreads=1", "-dUseFastColor=true", "-dNOGC", "-dUseCropBox", f"-sOutputFile=" + output_path, input_path]
    try:
        logger.info(f"Running AWS-optimized Ghostscript compression: {compression_level}")
        env = os.environ.copy()
        env.update({"OMP_NUM_THREADS": "1", "OPENBLAS_NUM_THREADS": "1", "MKL_NUM_THREADS": "1", "VECLIB_MAXIMUM_THREADS": "1", "NUMEXPR_NUM_THREADS": "1", "GS_THREADS": "1"})
        result = subprocess.run(gs_command, capture_output=True, timeout=400, env=env, creationflags=creationflags)
        if result.returncode != 0:
...
def compress_with_fallback(input_path: str, output_path: str) -> bool:
    try:
        if platform.system() == "Windows":
            ...
            creationflags = 0
        fallback_command = base_cmd + ["-dNOPAUSE", "-dBATCH", "-dQUIET", "-sDEVICE=pdfwrite", "-dPDFSETTINGS=/screen", "-dCompatibilityLevel=1.4", "-dColorImageResolution=100", "-dGrayImageResolution=100", "-dMonoImageResolution=150", "-dColorImageDownsampleType=/Average", "-dGrayImageDownsampleType=/Average", "-dMonoImageDownsampleType=/Subsample", "-dNumRenderingThreads=1", f"-sOutputFile=" + output_path, input_path]
        result = subprocess.run(fallback_command, capture_output=True, timeout=300, env={**os.environ, "OMP_NUM_THREADS": "1", "GS_THREADS": "1"}, creationflags=creationflags)
...
def cleanup_compression_estimation_files(task_id: str):
    try:
        patterns = [f"{task_id}_*", f"compressed_{task_id}_*", f"*{task_id}_*estimation.pdf"]
        ...
        while True:
            ...
            if file_size > 0:
                ...
                if chunk_number % 10 == 0 or uploaded_size % (1024 * 1024) == 0:
                    await progress_tracker.update_progress(task_id, int(progress), f"Uploading to disk... ({uploaded_size / (1024 * 1024):.1f} MB)", "uploading")
...
async def process_estimation_sequential_disk(task_id: str, file_path: str, filename: str):
    try:
        ...
        estimation_results[task_id] = {"estimates": estimates, "original_size_mb": round(original_size_mb, 2), "used_s3": False, "sequential_processing": True}
...
def cleanup_orphaned_files():
    try:
        ...
        for base_dir in root_directories:
            ...
            for root, dirs, files in os.walk(str(base_dir), topdown=False):  # topdown=False is important
                for filename in files:
                    file_path = Path(root) / filename
                    total_scanned += 1
                    ...
                    try:
                        ...
                        file_path.unlink()
                        cleaned_files += 1
                        logger.info(f"🗑️ Deleted old file: {file_path.relative_to(base_dir)}")
                ...
                for dir_name in dirs[:]:   # copy list to avoid modification issues
                    ...
                    try:
                        if dir_path.exists() and not any(dir_path.iterdir()):  # if empty
                            dir_path.rmdir()
                            cleaned_dirs += 1
                            logger.info(f"🗑️ Deleted empty folder: {dir_path.relative_to(base_dir)}")
                    except Exception as e:
                        logger.error(f"Failed to delete folder {dir_path}: {e}")

        logger.info(f"🎯 Cleanup Finished! Files deleted: {cleaned_files} | Empty folders deleted: {cleaned_dirs} | Scanned: {total_scanned}")
...
def convert_pdf_to_word_disk_based(pdf_file_path: Path, task_id: str, max_retries: int = 1) -> Optional[bytes]:
...
def convert_pdf_to_word_local_fallback(pdf_file_path: Path, task_id: str) -> Optional[bytes]:
...
def save_video_metadata(metadata):
    try:
        s3_client.put_object(Bucket=BUCKET_NAME, Key=METADATA_KEY, Body=json.dumps(metadata, indent=2), ContentType='application/json')
...
async def list_videos():
    try:
        ...
        if "Contents" in response:
            video_items = [obj for obj in response["Contents"] if obj["Key"].lower().endswith((".mp4", ".webm", ".ogg", ".mkv", ".avi", ".mov"))]
            ...
            for obj in video_items:
                ...
                content_type_map = {'mp4': 'video/mp4', 'webm': 'video/webm', 'ogg': 'video/ogg', 'mkv': 'video/x-matroska', 'avi': 'video/x-msvideo', 'mov': 'video/quicktime'}
                # ✅ Secure presigned URL instead of direct URL
                secure_url = get_secure_download_url(key, expires_in=3600)
                if not secure_url:
                    continue
                videos.append({"id": video_id, "name": filename, "url": secure_url, "raw_url": secure_url, "description": sanitize_input(description, max_length=500), "type": content_type_map.get(ext, 'video/mp4'), "format": ext.upper(), "size": obj.get('Size', 0), "uploaded": obj["LastModified"].strftime("%Y-%m-%d %H:%M"), "has_description": bool(description and description.strip())})
...
async def create_razorpay_order(request: Request):
    try:
        ...
        if amount <= 0:
            return JSONResponse(status_code=400, content={"success": False, "error": "Invalid amount"})
        amount_in_paise = int(amount * 100)
        order_data = {"amount": amount_in_paise, "currency": "INR", "payment_capture": 1, "receipt": f"receipt_{int(time.time())}", "notes": {"payment_for": "Website Services", "customer": "Guest User"}}
        order = client.order.create(data=order_data)
        logger.info(f"✅ Order created successfully: {order['id']}")
        response_data = {"success": True, "order_id": order["id"], "amount": amount_in_paise, "currency": "INR", "key": RAZORPAY_KEY_ID}
...
async def get_payment_details(request: Request, payment_id: str):
    try:
        ...
        return JSONResponse({"success": True, "payment_id": payment.get('id'), "order_id": payment.get('order_id'), "amount": payment.get('amount', 0) / 100, "status": payment.get('status'), "method": payment.get('method'), "utr": utr, "bank_reference": payment.get('bank_reference'), "created_at": datetime.fromtimestamp(payment.get('created_at', 0)).strftime('%Y-%m-%d %H:%M:%S')})
    except Exception as e:
...
async def razorpay_webhook(request: Request):
    try:
        ...
        expected_signature = hmac.new(RAZORPAY_KEY_SECRET.encode(), body, hashlib.sha256).hexdigest()
        if signature == expected_signature:
            ...
            if webhook_data.get('event') == 'payment.captured':
                ...
                pending_payments[payment_id] = {'paid': True, 'order_id': order_id, 'amount': amount, 'timestamp': datetime.now().isoformat()}
...
async def get_payment_receipt(amount: float = 0, method: str = "upi"):
    ...
    method_emoji = {'razorpay': '💳 Razorpay', 'upi': '📱 UPI', 'card': '💳 Card', 'wallet': '🏦 Wallet'}
    return JSONResponse({"success": True, "greeting": greeting, "message": message, "amount": amount, "method": method_emoji.get(method, method), "timestamp": now_ist.strftime('%Y-%m-%d %H:%M:%S'), "timezone": "IST"})
